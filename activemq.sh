...
COMMANDLINE_ARGS="$@"
EXEC_OPTION=""
trap ' show_Reason 'SIGHUP'' 1
trap ' show_Reason 'SIGINT'' 2
trap ' show_Reason 'SIGQUIT'' 3
trap ' show_Reason 'SIGXCPU'' 24
...
case "$1" in
  ...
  restart)
    ...
    #  $0 stop
    #fi
    #echo "`date "+%Y-%m-%d %H:%M:%S"` begin test sleep 3m ... ..." >>/logs/MQrestart.log
    #sleep 3m
    #echo "`date "+%Y-%m-%d %H:%M:%S"` end test sleep 3m ... ..." >>/logs/MQrestart.log

    #$0 status
    #$0 start
    #$0 status
    while(checkRunning); do
        echo "[`date "+%Y-%m-%d %H:%M:%S"`] before stop ... ..."
        $0 stop
        echo "[`date "+%Y-%m-%d %H:%M:%S"`] the amq server would have been stoped ! now sleep 30s begin ... ..."
        sleep 30s
        echo "[`date "+%Y-%m-%d %H:%M:%S"`] sleep 30s end ... ..."
        stoploop=`expr $stoploop + 1`
        if [ $stoploop -gt 10 ]; then
                break
        fi 
    done
    $0 status
    ...
    until(checkRunning); do
        echo "[`date "+%Y-%m-%d %H:%M:%S"`] before start ... ..."
        $0 status
        $0 start
        $0 status
        echo "[`date "+%Y-%m-%d %H:%M:%S"`] the amq server would have been started ! now sleep 30s begin ... ..."
        sleep 30s
        echo "[`date "+%Y-%m-%d %H:%M:%S"`] sleep 30s end ... ..."
        startloop=`expr $startloop + 1`
        if [ $startloop -gt 10 ]; then
                break
        fi 
    done
